...
func main() {
    ...
    router.HandleFunc("/api/admin/users", handlers.GetAllUsers(db)).Methods("GET")
    router.HandleFunc("/api/admin/users/{id}/role", handlers.UpdateUserRole(db)).Methods("PUT")

    // Создаем и настраиваем CORS middleware
    corsMiddleware := cors.New(cors.Options{
        AllowedOrigins:   []string{"*"},
        AllowedMethods:   []string{"GET", "POST", "PUT", "DELETE", "OPTIONS"}, // Добавил PUT и DELETE
